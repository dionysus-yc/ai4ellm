...
class CodeFileInstance:
    def __init__(self, repo_path, file_path, target_encoding="utf-8", zf=None):
        if isinstance(file_path, PosixPath):
            ...
            with zf.open(file_path.filename, 'r') as r:
                file_bytes = r.read()
...
class Zipfile2JsonL:
    def __init__(self, output_root, chunk_counter, target_encoding="utf-8", clean_src_file=False, plateform="github",
                 author=""):
    ...
    def temp2jsonl(self):
        ...
        if size1 + size2 <= self.max_jsonl_size:
            # 如果写入后不超过限制，就直接写入
            with open(self.temp_name, "r", encoding="utf-8") as r, open(self.get_jsonl_file(), "a",
                                                                        encoding="utf-8") as a:
                a.write(r.read())
